...
// all sunscribe user
module.exports.all_subscription_user = async (req, res) => {
  try {

    const all_subscribe_users = await subscriptionModel.find({ status: "active" }).sort({ createdAt: -1 })
    return res.send({ status: 1, message: "All subscription users.", data: all_subscribe_users })

  }
  catch (error) {
    return res.send({ status: 0, message: "Server error please try again" })
  }
