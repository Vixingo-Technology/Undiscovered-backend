...
module.exports.createVideo = async (req, res) => {
  ...
  console.log("video", video, "req.body ", req.body)
...
module.exports.createVideoLink = async (req, res) => {
  let { title, description, featuredPlayer, isfeatured, link, type } = req.body;
...
// pin video
module.exports.pinVideo = async (req, res, next) => {
  try {
    const video_id = req.body.video_id
    const isfeatured = req.body.isfeatured
    if (!video_id) {
      return res.send({ status: 0, message: "Video id is required." })
    }
    const findVideo = await videoModel.findOne({ _id: video_id })

    if (!findVideo) {
      return res.send({ status: 0, message: "Video not found." })
    }
    else {
      findVideo.isfeatured = isfeatured
      await findVideo.save()
      const action = isfeatured ? "pinned" : "unpinned";
      return res.send({ status: 1, message: `Video ${action} successfully`, })
    }
  }
  catch (error) {
