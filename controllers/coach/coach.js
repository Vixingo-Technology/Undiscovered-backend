...
module.exports.createPlayer = async (req, res) => {
  ...
  try {
    ...
    if (!id) {
      return res.send({ status: 0, message: "Id id required." });
    }
    ...
    if (profile) {
      ...
      if (video3) playerUpdateFields.video3 = video3;
      if (video4) playerUpdateFields.video4 = video4;
      if (starRating) playerUpdateFields.starRating = starRating;
      if (is_shown) playerUpdateFields.is_shown = is_shown;
      playerUpdateFields.is_accept_reject = 3
      ...
      try {
        ...
        console.log(player, "YE UPDATE HUA");
        try {
